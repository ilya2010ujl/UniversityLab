...
int main()
{
    ...
    std::function<qreal(const QVector<qreal>&)> barrier = [=](const QVector<qreal>& x) -> qreal
    {
        return qPow(qMax(0.0, -x[0]),p) + qPow(qMax(0.0, -x[1]),p)
                + qPow(qAbs(x[0] + x[1] - 2), p);
    };

    // метод барьерных функций
    while(qAbs(r * barrier(result)) > e)
    {
